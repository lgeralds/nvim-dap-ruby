...
local function run_cmd(cmd, args, for_current_line, for_current_file, relative_file, error_on_failure)
  local handle
  local pid_or_err
  ...
  if for_current_line then
    table.insert(args, filename .. ":" .. vim.fn.line("."))
...
local function setup_ruby_configuration(dap)
  local base_config = { type = "ruby", request = "attach", options = { source_filetype = "ruby" }, error_on_failure = true, localfs = true, relative_file = false }
